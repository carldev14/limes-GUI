...
export default function FormTemplatesUI() {
    ...
    function isSuccess() {
        if (login_response && login_response.success === true) {
            router.push('/p/home');
        }
        ...
        <div className="p-6 w-full md:w-[28%] flex flex-col items-center md:justify-between gap-2">
            ...
            <section className="flex flex-col gap-9 items-center w-full">
                <section className="flex flex-col gap-6 items-center">
                    <h1 className={`${BoldPoppins.className} text-xl text-black/80`}>Welcome back!</h1>
                ...
                <form onSubmit={handleSubmit(submitNow)} className="flex flex-col gap-4 w-full p-1">
                    <section className="flex flex-col gap-3">
                        ...
                        <Link href={'/forgot-password'} className={`text-xs mx-1 text-red-600 text-start ${RegularPoppins.className}`}>Forgot Password?</Link>
                    </section>



                    {login_response && login_response.success ? (
                        <p className="bg-green-200 p-3 text-sm text-green-800 rouned-md">{login_response.message}</p>
                    ) : login_response && !login_response.success ? (
                        <p className="bg-red-200 p-3 text-sm text-red-800 rounded-md">{login_response.message}</p>
                    ) : null}


                    <button type="submit" className={`text-sm rounded-xl p-3 bg-green-700 text-white ${RegularPoppins.className}`}>
                        Login
                    </button>
                    <Link href={'/register'} className={`text-sm text-black/90 text-center ${RegularPoppins.className}`}>Don't have an account? Click me</Link>
                </form>
            </section>
        </div>
    );
